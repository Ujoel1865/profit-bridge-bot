...
from user_store import process_deposit_if_eligible
import logging

def handle_mint(message, context=None):  # context is not used for telebot
    telegram_id = message.from_user.id

    # Fetch user wallet
    wallet_data = get_wallet_by_user(telegram_id)
    if not wallet_data:
        message.reply_text("🚫 Wallet not found. Please register first.")
        return
    ...
    try:
        ...
        message.reply_text("⚠️ Failed to refresh your balance. Please try again later.")
        return

    # Ensure user has at least 50 USDT
    ...
    if result.get("success"):
        ...
        message.reply_text(f"❌ Sweep failed: {error_msg}")
